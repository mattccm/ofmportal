...
// Validation, rendering, and utility functions for the template system
...
export async function getTemplateUsageCount(templateId: string): Promise<number> {
  const count = await db.contentRequest.count({
    where: { templateId },
  });
  return count;
}

export async function getTemplatesWithUsage(agencyId: string) {
